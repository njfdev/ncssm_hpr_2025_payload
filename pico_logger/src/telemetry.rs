...
pub fn format_status(bmp390: bool, lsm6dsox: bool, lis3mdl: bool, ens160: bool) -> String<128> {
    let mut s = String::new();
    let _ = write!(s, "BMP390:{} LSM6DSOX:{} LIS3MDL:{} ENS160:{}\r\nOK\r\n",
        if bmp390 { "OK" } else { "ERR" },
        if lsm6dsox { "OK" } else { "ERR" },
        if lis3mdl { "OK" } else { "ERR" },
        if ens160 { "OK" } else { "ERR" },
    );
    s
}

/// Format telemetry data packet - uses short format to fit in 64-byte USB packet
pub fn format_telemetry(counter: u32, data: &SensorData) -> String<80> {
    let mut s = String::new();
    // Compact format: #N,P,T,ax,ay,az,gx,gy,gz,mx,my,mz,aqi,tvoc,eco2
    let _ = write!(s,
        "#{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\r\n",
        counter,
        data.pressure_raw,
